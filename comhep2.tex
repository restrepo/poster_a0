...
\begin{frame}
 ...
 For Dirac neutrino masses: we require to intruce at least one SM-singlet heavy Dirac fermión
 \begin{align}
...
\only<1->{\put(0,100){ $\frac{1}{\Lambda} L \cdot H L \cdot H$  (
    \only<1>{$n$}\only<2->{1}-loop) }}%
\only<1>{\put(-20,80){ {\scriptsize $n=1$: Bonnet, \emph{et al}, arXiv:1204.5862 [JHEP]} }}%
\only<1>{\put(-20,70){ {\scriptsize $n=2$: Aristizabal, \emph{et al},  arXiv:1411.7038 [JHEP]} }}%
\only<1>{\put(320,150){\large \color{white} $m_{\nu_i}\ne 0$}}%
\only<1>{\put(330,100){\large \color{white} $\Downarrow$}}%
\only<1>{\put(180,50){\color{white} Distinguish between simplified dark Matter models}}%
...
\only<2->{\put(120,40){ \begin{overpic}[scale=0.15]{brickssigma}\end{overpic}}}%
\only<2->{\put(190,40){ \begin{overpic}[scale=0.15]{brickssG}\end{overpic}}}%
