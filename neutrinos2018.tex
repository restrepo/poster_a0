...
\pgfpagesuselayout{8 on 1}[a0paper, border shrink=5mm]
...
If $\color{Green}\nu_i\ne 0$
  ...
  \begin{picture}(100,10)
    \put(10,270){{\Huge \alert{Colored Scotogenic.}  }}
    \put(350,270){
      \begin{minipage}{0.5\linewidth}
        \centering
        {\large\textbf{\alert{Diego Restrepo} }  }\\
        \alert{Instituto de Física}\\
        \alert{Universidad de Antioquia}
      \end{minipage}
    }
    \put(700,270){
      \begin{minipage}{0.5\linewidth}
        \centering
        \alert{\textbf{Focus on}: arXiv:1803.08528 [PRD]}\\
        \alert{\textbf{In collaboration with}:} \\
        Mario Reig, Jose Valle (IFIC Valencia),\\
        Oscar Zapata (UdeA)
% \arxiv{arXiv:1803.08528}
% \collaborators{Mario Reig, Jose Valle (IFIC Valencia), Oscar Zapata (UdeA) }
      \end{minipage}
  ...
  \begin{picture}(100,10)
    \put(-200,-60){
      \begin{minipage}{2.0\linewidth}
        if we impose $\operatorname{U}(1)_{B-L}$ to be local:\hspace{2cm}
%   \begin{center}
      \rowcolors{1}{RoyalBlue!20}{}    
  \begin{tabular}{rlll}
...
\begin{frame}
  \frametitle{
    \only<1>{Exotic $\left( \nu_R \right)^{\dagger}$ with $\nu\ne-1$,
     ...
     \only<2>{The model: \textit{colored scotogenic}}%
  }
\def\pscale{0.6}
\def\px{0}
\def\py{140}
\begin{picture}(320,250)
\only<1>{\put(\px,\py){%\includegraphics[scale=\pscale]{dm3}
    \begin{overpic}[scale=\pscale]{dm3}
    \end{overpic}
  }}%
\only<2>{\put(\px,\py){\includegraphics[scale=\pscale]{dm4}}}%
%%% circle
\FPeval\vx{\px+52}%
\FPeval\vy{\py+3}%
\only<1>{\put(\vx,\vy){\tikz \draw[red,thick,rounded corners] (0,0) rectangle (0.5,0.5);} }
\FPeval\vxr{\vx+73}%
\only<1>{\put(\vxr,\vy){\tikz \draw[red,thick,rounded corners] (0,0) rectangle (0.5,0.5);} }
\FPeval\vxr{\vx+\vy}%
\only<1>{\put(\vxr,\vy){\tikz \draw[red,thick,rounded corners] (0,0) rectangle (0.5,0.5);} }
\FPeval\vxr{\vx+73}%
\FPeval\vyr{\vy+58}%
\only<1>{\put(\vxr,\vyr){\tikz \draw[red,thick,rounded corners] (0,0) rectangle (0.5,0.5);} }
\FPeval\sx{\px+270}%
\FPeval\sy{\py+60}%
\only<1>{\put(\sx,\sy){
...
\only<1->{\put(0,80){
    \begin{columns}
      \begin{column}{0.55\textwidth}
        ...
        \end{align*}
        \begin{align*}
          \only<1>{\boldsymbol{N}_c=\boldsymbol{1}\,.}%
          \only<2>{\color{red}\boldsymbol{N}_c=\boldsymbol{8}\,.}%
        \end{align*}
      \end{column}
...
\hline
 ...
 $\left( \nu_{Ri} \right)^{\dagger}$ & $\color{Green}\nu$ & $ \left(\mathbf{1},\mathbf{1}\right)_0$ \\ 
    \only<1>{$\psi_L$}\only<2->{$\color{red}\mathcal{Q}_L$}    & $\color{red}-r$ & $ \left(\only<1>{\mathbf{N}_c}\only<2>{{\color{red}\mathbf{N}_c}},\mathbf{1}\right)_0$\\
 \only<1>{$\left( \psi_R \right)^{\dagger}$}\only<2->{$\color{red}\left( \mathcal{Q}_R \right)^{\dagger}$}   & $\color{red}r$ & $ \left(\only<1>{\mathbf{N}_c}\only<2>{{\color{red}\mathbf{N}_c}},\mathbf{1}\right)_0$ \\
    $\sigma_a$                           &  ${\color{Green}\nu}-\color{red}r$   & $ \left(\only<1>{\mathbf{N}_c}\only<2>{{\color{red}\mathbf{N}_c}},\mathbf{1}\right)_0$ \\
    $\eta_a$                        &  $1-\color{red}r$   & $ \left(\only<1>{\mathbf{N}_c}\only<2>{{\color{red}\mathbf{N}_c}},\mathbf{2}\right)_{1/2}$ \\
    \invisible<2>{$S$}                        &  \invisible<2>{ ${\color{Green}\nu}-1$ }  & $  \invisible<2>{ \left(\mathbf{N}_c,\mathbf{2}\right)_{1/2}$ }\\  
  \end{tabular}
      ...
      \begin{minipage}{1.4\linewidth}
        \begin{columns}
          \begin{column}{0.5\linewidth}
            To have at least a rank 2 neutrino mass matrix we need:\\
            At least two sets of scalars $\eta_a$, $\sigma_a$
          \end{column}
          \begin{column}{0.5\linewidth}
  \begin{align*}
...
%   \item<1-> To have at least a rank 2 neutrino mass matrix we need either:
%   \begin{itemize}
%   \item At least two heavy Dirac fermions $\mathcal{Q}_a$, $a=1,2,\ldots$
%   \item \only<1>{At least two sets of scalars $\eta_a$, $\sigma_a$}%
%     \only<1->{\color{red}At least two sets of scalars $\eta_a$, $\sigma_a$}%
%   \end{itemize}
% \item<1> 
%   \begin{align*}
%     \mathcal{L}\supset&\left[ {\color{red}  M_{\mathcal{Q}}\, \left( \mathcal{Q}_R \right)^{\dagger} \mathcal{Q}_L}
...
\only<1->{\put(280,180){\begin{overpic}[scale=0.15]{Q}\end{overpic}}}%
\only<1->{\put(110,-30){ \begin{overpic}[scale=0.15]{sigmas}\end{overpic}}}%
\only<1->{\put(220,-20){ \begin{overpic}[scale=0.15]{etas}\end{overpic}}}%
\end{picture}
\end{frame}
...
\begin{picture}(100,10)
    \put(-140,-8){
      \begin{minipage}{1.0\linewidth}
        \includegraphics[scale=0.14]{relden}
        \tiny{From: \url{http://bit.ly/Mitridate_53d_53rd_Rencontres_de_Moriond} }
      \end{minipage}
}
\end{picture}
...
\begin{frame}
  \frametitle{Direct detection \& Collider Physics}
  \begin{columns}
    \begin{column}{0.6\linewidth}
  \includegraphics[scale=0.45]{sigmaSI}
    \end{column}
    \begin{column}{0.3\linewidth}
      ...
      \end{block}
      \begin{align*}
        \sqrt{s}=&65\ \text{TeV needed to discover }\\
         M_{\mathcal{Q}}=&9.5\ \text{TeV}\,.
      \end{align*}
    \end{column}
  \end{columns}
...
\begin{frame}
  ...
  \begin{block}<1>{Dirac neutrino masses and DM}
    \begin{itemize}
